...
(require racket/string
         rackunit
         web-server-compress/zstd
         web-server/http/response-structs
         "support/helpers.rkt"
         "support/zstd.rkt")

(define (collect-response-body/decompressed resp)
  (zstd-decompress (collect-response-body resp)))

(module+ test
  (require rackunit/text-ui)
  (run-tests
   (test-suite "wrap-zstd-compress"

     (test-case "adds Content-Encoding: zstd when client accepts zstd"
       (define handler (wrap-zstd-compress (make-text-handler "hello")))
       (define resp (handler (make-req "gzip, deflate, zstd")))
       (check-equal? (response-header-value resp #"Content-Encoding") #"zstd"))

     (test-case "no Content-Encoding when client does not accept zstd"
       (define handler (wrap-zstd-compress (make-text-handler "hello")))
     ...
     (test-case "quality weights are stripped correctly"
       (define handler (wrap-zstd-compress (make-text-handler "hello")))
       (define resp (handler (make-req "gzip;q=1.0, zstd;q=0.5")))
       (check-equal? (response-header-value resp #"Content-Encoding") #"zstd"))
     ...
     (test-case "round-trip: compressed body decompresses to original"
       (define text "hello, zstd world!")
       (define handler (wrap-zstd-compress (make-text-handler text)))
       (define resp (handler (make-req "zstd")))
     ...
     (test-case "custom #:level round-trips correctly"
       ...
       (define handler (wrap-zstd-compress (make-text-handler text) #:level 6))
       (define resp (handler (make-req "zstd")))
       (define decompressed (bytes->string/utf-8 (collect-response-body/decompressed resp)))
       (check-equal? decompressed text))
     ...
     (test-case "mid-stream flush produces decodable output (SSE scenario)"
       ...
       (define handler
         (wrap-zstd-compress (lambda (_req)
                               (response 200
                                         #"OK"
                                         (current-seconds)
                                         #"text/event-stream"
       ...
       (define result (bytes->string/utf-8 (zstd-decompress compressed)))
     ...
     (test-case "compresses text/html responses"
       (define handler (wrap-zstd-compress (make-handler-with-mime "hello" #"text/html")))
       (define resp (handler (make-req "zstd")))
       (check-equal? (response-header-value resp #"Content-Encoding") #"zstd"))

     (test-case "compresses application/json responses"
       (define handler (wrap-zstd-compress (make-handler-with-mime "{}" #"application/json")))
       (define resp (handler (make-req "zstd")))
       (check-equal? (response-header-value resp #"Content-Encoding") #"zstd"))

     (test-case "compresses text/event-stream responses"
       (define handler
         (wrap-zstd-compress (make-handler-with-mime "data: hi\n\n" #"text/event-stream")))
       (define resp (handler (make-req "zstd")))
       (check-equal? (response-header-value resp #"Content-Encoding") #"zstd"))

     (test-case "does not compress image/jpeg responses"
       (define handler (wrap-zstd-compress (make-handler-with-mime "fake" #"image/jpeg")))
       (define resp (handler (make-req "zstd")))
       (check-false (response-header-value resp #"Content-Encoding")))

     (test-case "does not compress image/png responses"
       (define handler (wrap-zstd-compress (make-handler-with-mime "fake" #"image/png")))
       (define resp (handler (make-req "zstd")))
       (check-false (response-header-value resp #"Content-Encoding")))

     (test-case "does not compress application/zip responses"
       (define handler (wrap-zstd-compress (make-handler-with-mime "fake" #"application/zip")))
       (define resp (handler (make-req "zstd")))
       (check-false (response-header-value resp #"Content-Encoding")))

     (test-case "compresses responses with no Content-Type"
       (define handler (wrap-zstd-compress (make-handler-with-mime "hello" #f)))
       (define resp (handler (make-req "zstd")))
       (check-equal? (response-header-value resp #"Content-Encoding") #"zstd"))

     (test-case "compresses text/html with charset parameter"
       (define handler
         (wrap-zstd-compress (make-handler-with-mime "hello" #"text/html; charset=utf-8")))
       (define resp (handler (make-req "zstd")))
       (check-equal? (response-header-value resp #"Content-Encoding") #"zstd"))

     (test-case "custom #:compress? can disable compression"
       (define handler
         (wrap-zstd-compress (make-text-handler "hello") #:compress? (lambda (_resp) #f)))
       (define resp (handler (make-req "zstd")))
       (check-false (response-header-value resp #"Content-Encoding")))

     (test-case "custom #:compress? can enable compression for normally-skipped types"
       (define handler
         (wrap-zstd-compress (make-handler-with-mime "fake" #"image/jpeg")
