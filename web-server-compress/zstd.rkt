#lang racket/base

(require racket/contract/base
         web-server/http
         "private/zstd-ffi.rkt"
         "private/accept-encoding.rkt"
         "private/compressible.rkt")

(provide level/c
         (contract-out [wrap-zstd-compress
                        (->* [(-> request? response?)]
                             [#:level level/c #:compress? (-> response? boolean?)]
                             (-> request? response?))]))

(define (wrap-zstd-compress handler #:level [level 3] #:compress? [compress? #f])
  (define effective-compress?
    (or compress? (lambda (resp) (compressible-mime-type? (response-mime resp)))))
  (lambda (req)
    (define resp (handler req))
    (cond
      [(and (accepts-encoding? req 'zstd) (effective-compress? resp))
       (define original-output (response-output resp))
       (struct-copy response
                    resp
                    [headers
                     (list* (make-header #"Content-Encoding" #"zstd")
                            (make-header #"Vary" #"Accept-Encoding")
                            (response-headers resp))]
                    [output
                     (lambda (raw-out)
                       (define out (open-zstd-output raw-out #:level level #:close? #f))
                       (dynamic-wind void
                                     (lambda () (original-output out))
                                     (lambda () (close-output-port out))))])]
      [else resp])))
